...
from oelsmann24_verticallandmotion.vlm_postprocess import (
   vlm_postprocess
)
...
def main(pipeline_id: str,
    ...
    if baseyear < 2000:
       raise ValueError("Baseyear cannot be less than year 2000. Baseyear value received: %s", baseyear)
    if baseyear > 2300:
       raise ValueError("Baseyear cannot be greater than year 2300. Baseyear value received: %s", baseyear)
    if pyear_start < 2000:
       raise ValueError("Projection start year cannot be less than year 2000. Projection start year value received: %s", pyear_start)
    if pyear_end > 2300:
       raise ValueError("Projection stop year cannot be greater than year 2300. Projection stop year value received: %s", pyear_end)
    if pyear_start >= pyear_end:
       raise ValueError("Projection start year must be less than projection stop year. Projection start year value received: %s, projection stop year value received: %s", pyear_start, pyear_end)
    if pyear_step < 1:
         raise ValueError("Projection year step must be at least 1 year. Projection year step value received: %s", pyear_step)
